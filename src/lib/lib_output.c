...
struct output *check_monitor(lua_State *L, int narg) {
    void **ud = luaL_checkudata(L, narg, CONFIG_MONITOR);
    luaL_argcheck(L, ud != NULL, 1, "`monitor' expected");
    return *(struct output **)ud;
}

static void create_lua_monitor(lua_State *L, struct output *m) {
    struct output **user_monitor = lua_newuserdata(L, sizeof(struct output *));
    *user_monitor = m;
...
{
    struct output *m = check_monitor(L, 1);
    lua_pop(L, 1);
