...
struct wlr_list get_combined_visible_lists(struct workspace_selector *ws_selector)
{
    ...
    struct wlr_list *ref_list =
        get_visible_lists(get_workspace(selected_monitor->ws_selector.ws_id));

    for (int i = 0; i < ref_list->length; i++) {
        for (int j = 0; j < workspaces.length; j++) {
            struct workspace *ws = workspaces.items[j];
            struct wlr_list *lists = get_visible_lists(ws);
...
{
    struct wlr_list workspaces = get_workspaces(&ws_selector->ids);
    struct wlr_list res_list = combine_lists(&workspaces, get_tiled_list);
    return res_list;
}
...
{
    struct wlr_list workspaces = get_workspaces(&ws_selector->ids);
    struct wlr_list res_list = combine_lists(&workspaces, get_hidden_list);
    return res_list;
}
