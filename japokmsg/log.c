...
void _sway_abort(const char *format, ...) {
    ...
    _sway_vlog(JAPOKWM_ERROR, format, args);
    va_end(args);
...
static sway_log_importance_t log_importance = JAPOKWM_ERROR;
static struct timespec start_time = {-1, -1};

static const char *verbosity_colors[] = {
    [JAPOKWM_SILENT] = "",
    [JAPOKWM_ERROR ] = "\x1B[1;31m",
    [JAPOKWM_INFO  ] = "\x1B[1;34m",
    [JAPOKWM_DEBUG ] = "\x1B[1;90m",
};
...
void log_init(sway_log_importance_t verbosity, terminate_callback_t callback) {
    init_start_time();
